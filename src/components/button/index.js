...
export default function Button({ children, href = "", styleType = "primary", type = "button", classes = ""}) {
    const [isMouseDown, setIsMouseDown] = useState(false);
    const [isHovering, setIsHovering] = useState(false);
    const buttonStyle = styles[styleType];
    ...
    return (
        <button
            type={type}
            class={`rounded-2xl font-semibold ${classes} ${buttonStyle["base"]} 
            ${isHovering ? (isMouseDown ? buttonStyle["active"] : buttonStyle["hover"]) : buttonStyle["regular"]}`}
