...
const supportUsCards = [
    ...
    },
    {
        ...
        imageAlt: "Graphic of purple hand wrapped in blue string meeting with beige hand with blue blob coming out of palm",
        button: "Get tickets",
        buttonLink: "",
    ...
    },
    {
        ...
        description: "Make a one-time or monthly donation to help run Scambi Festival.",
        ...
        <div className={`${classes} space-y-10`}>
            ...
            <div className='w-full flex flex-col items-center space-y-10 mx-5 lg:items-stretch lg:space-y-0 lg:space-x-7 lg:flex-row lg:justify-center xl:space-x-14'>
                {supportUsCards.map((card, index) =>
                    <Card
                        classes={`bg-${card.background} w-4/5 p-10 flex flex-col justify-between space-y-3 shadow-xl lg:space-y-5 lg:w-72 xl:w-1/4`}
                        key={`supportus-${index}`}
                    >
                        <h4>{card.title}</h4>
